  ...
  return (
    <article className='group w-full h-44 rounded-lg border border-gray-700 overflow-hidden hover:scale-105 transition-all duration-150'>
      ...
      <footer className='relative bottom-16'>
        <ButtonGroup>
          <Button href={repository.deploy} target='_blank' variant='contained' startIcon={WINDOW_ICON(32, 32, 'Deploy website')} rel='noopener' component={Link}>
            Deploy
          </Button>
          <Button href={repository.htmlUrl} target='_blank' variant='contained' endIcon={CODE_ICON(32, 32, 'View source code')} color='secondary' rel='noopener' component={Link}>
            Source
          </Button>
        </ButtonGroup>
      </footer>
